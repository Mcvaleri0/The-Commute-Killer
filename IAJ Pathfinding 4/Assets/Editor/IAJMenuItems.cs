...
public class IAJMenuItems  {
    ...
    private static void CreateClusterGraph()
    {
        ...
        {
            var row = ScriptableObject.CreateInstance<GatewayDistanceTableRow>();
        ...
        for (var i = 0; i < gateways.Length; i++)
        {
            ...
            for (var j = i; j < gateways.Length; j++)
            {
                ...
                if (i != j) {
                    ...
                    if (pathfindingManager.AStarPathFinding.InProgress)
                    {
                        var finished = pathfindingManager.AStarPathFinding.Search(out solution);

                        while (!finished)
                        {
                            finished = pathfindingManager.AStarPathFinding.Search(out solution);
                        }
                    ...
                    var entry = ScriptableObject.CreateInstance<GatewayDistanceTableEntry>();
