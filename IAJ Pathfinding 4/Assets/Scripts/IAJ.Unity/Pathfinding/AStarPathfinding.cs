...
namespace Assets.Scripts.IAJ.Unity.Pathfinding
{
    public class AStarPathfinding
    {
        ...
        private float StartProcessingTime { get; set; }
        ...
        protected virtual void ProcessChildNode(NodeRecord parentNode, NavigationGraphEdge connectionEdge, int edgeIndex)
        {
            ...
            if (Open.SearchInOpen(childNode) == null && Closed.SearchInClosed(childNode) == null)
            {
                Open.AddToOpen(childNode);
            }
            else
            {
                var nodeIn = Open.SearchInOpen(childNode);

                if (nodeIn != null && nodeIn.fValue > childNode.fValue)
                {
                    Open.Replace(nodeIn, childNode);
                    return;
                }

                nodeIn = Closed.SearchInClosed(childNode);

                if (nodeIn != null && nodeIn.fValue > childNode.fValue)
                {
                    Closed.RemoveFromClosed(nodeIn);
                    Open.AddToOpen(childNode);
                }
            }
        }
        ...
        public bool Search(out GlobalPath solution, bool returnPartialSolution = false)
        {
            ...
            if(openCount > MaxOpenNodes)
            {
                MaxOpenNodes = openCount;
            }
        ...
        protected GlobalPath CalculateSolution(NodeRecord node, bool partial)
        {
            ...
            Vector3 pos1 = currentNode.node.LocalPosition;
            ...
            while (currentNode.parent != null)
            {
                ...
                pos2 = currentNode.node.LocalPosition;
                path.PathPositions.Add(pos2);
                ...
                pos1 = currentNode.parent.node.LocalPosition;
