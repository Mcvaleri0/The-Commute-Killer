...
public class PathfindingManager : MonoBehaviour {
    ...
    private int gateDistX = 0;
    private int gateDistY = 0;
    ...
    public GatewayHeuristic GateH { get; set; }
	...
	void Awake ()
	{
        ...
        NavMeshPathGraph graph = NavigationManager.Instance.NavMeshGraphs[0];

        //IHeuristic heuristic = new ZeroHeuristic();
        //IHeuristic heuristic = new EuclideanDistanceHeuristic();
        this.GateH = new GatewayHeuristic();
        IHeuristic heuristic = this.GateH;
        ...
        /*this.Initialize(NavigationManager.Instance.NavMeshGraphs[0], 
                        new AStarPathfinding(NavigationManager.Instance.NavMeshGraphs[0], 
                                             new SimpleUnorderedNodeList(), 
                                             new SimpleUnorderedNodeList(), 
                                             heuristic));
        */

        this.Initialize(graph, new NodeArrayAStarPathFinding(graph, heuristic));

        mainCharController = mainCharacter.GetComponent<MainCharacterController>();
    }
    ...
    void Update () 
    {
		...
		if (Input.GetMouseButtonDown(0)) 
		{
			...
			if(this.MouseClickPosition(out position))
			{
                ...
                if (this.currentClickNumber == 1)
                {
                    ...
                    mainCharController.InitializePosition(position);
                }
                else
                {
        ...
        else if (currentSolution != null && mainCharController.Stopped)
        {
            mainCharController.InitializeMovement(currentSolution);
        }
	}
    ...
    public void OnGUI()
    {
        ...
        if (this.currentSolution != null)
        {
            ...
            text = "Nodes Visited: " + this.AStarPathFinding.TotalExploredNodes
                       ...
                       + "\nDistance: " + this.currentSolution.Length
                       + "\nFill (Explored - Path): " + (this.AStarPathFinding.TotalExploredNodes - currentSolution.NumberNodes);
    ...
    private void InitializePathFinding(Vector3 p1, Vector3 p2)
    {
        ...
        mainCharController.InitializePosition(startPosition);
    }
}
